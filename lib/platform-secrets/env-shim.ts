...
export async function installEnvShim(): Promise<{ hydrated: number; skipped: number }> {
  ...
  for (const key of keys) {
    ...
    // Do not clobber a value Vercel already set. An operator who pinned an
    // override in the dashboard meant it, and silently replacing it from the
    // vault is the kind of invisible divergence that costs a day to find.
    if (process.env[key] !== undefined && process.env[key] !== "") { skipped++; continue; }
