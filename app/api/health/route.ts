...
export async function GET(request: Request) {
  ...
  if (!url || !key) {
    ...
    try {
      ...
      // GoTrue service returned 503 upstream, ALL 133 APPS REPORTED DEGRADED AT ONCE
      // — while their databases were fine and every page served 200.
      //
      // That was my design fault, not Supabase's. Almost none of these apps depend on
      // GoTrue on a normal request; they depend on PostgREST. A health check should
      // test what the app actually needs, or one unrelated subsystem takes the whole
      // fleet's reporting down with it.
      //
      // platform_modules is a shared platform table readable with the app's own
      // publishable key, so this exercises the real dependency: HTTP, auth on the
      // key, PostgREST, and the database behind it.
